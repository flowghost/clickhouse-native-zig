...
- `Tuple`
- `LowCardinality`
- fixed-width types без отдельного специализированного wrapper
...
Если в схеме есть `AggregateFunction`, `Variant`, `Dynamic` или `Object`, лучше сначала прогнать конкретную схему на тестовом стенде или добить поддержку этих типов отдельно.
